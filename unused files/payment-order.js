...
function injectPOModalStyles() {
    ...
    style.innerHTML = `
        ...
        .net-amount { position: relative; right: 10%; }
...
function injectModifyModal() {
    ...
    // 1. Inject shared container CSS
    injectPOModalStyles();

    // 2. Inject specific styles for this modal's content
    if (!document.getElementById('po-modify-modal-styles')) {
        const style = document.createElement('style');
        style.id = 'po-modify-modal-styles';
        style.innerHTML = `
        ...
        <div class="po-modal" id="PO_MODAL_CONTENT">
            ...
            <div class="po-modal-body">
                ...
                <div class="po-form-side">
                    ...
                    <div class="po-group">
                        <div class="side-summary" style="width:100%; display:flex; flex-direction:column; gap:1px;">
 ...
 */
window.handleServiceBillModify = async function(rawBillData, refreshCallback) {
    if (!rawBillData || !rawBillData.orderNumber) {
        showAlert('Error: Invalid service bill data provided.', 'error');
        return;
    }
    ...
    if (btn && btn.tagName === 'BUTTON') {
        ...
        try {
            const response = await apiFetch(`/api/v1/payables/supplierBills/getSupplierBillDetails/${rawBillData.transactionType}`, { method: 'GET' });

            if (response && response.success && response.supplierBill) {
                if (typeof window.openServiceBillModifyModal === 'function') {
                    await window.openServiceBillModifyModal(response.supplierBill, refreshCallback);
                }
            } else {
                showAlert('Error: Failed to fetch bill details. ' + (response ? response.message : ''), 'error');
            }
        } catch (error) {
...
window.injectServiceBillModifyModal = function() {
    ...
    <div id="MODAL_ModifyServiceBill" class="po-modal-overlay" style="display: none; backdrop-filter: blur(3px);">
        <div id="MSB_ModalContent" class="po-modal" style="width: 95vw; height: 90vh; max-width: 1200px; display: flex; flex-direction: column;">
            ...
            <!-- Footer -->
            <div class="po-modal-footer">
                <button class="md-btn md-btn-primary" id="BTN_MSB_SaveChanges">Save Changes <i class="fa-solid fa-circle-check"></i></button>
...
window.addMSBLine = function(account) {
    ...
    if (modifyServiceBillState.data.some(b => b.accountName === account.name)) {
        // Optional: Highlight existing? For now, just ignore.
        return;
    }
    ...
    modifyServiceBillState.data.push({
        ...
        description: account.name, // Default desc
        amount: 0
    });
...
window.renderModifyServiceBillLines = function() {
    ...
    modifyServiceBillState.data.forEach((row, idx) => {
        ...
        tr.innerHTML = `
            ...
            <td style="display: none;">${(row.coaId)}</td>
            <td>${(idx + 1)}</td>
            <td>${row.accountName}</td>
            <td contenteditable="true" class="desc-cell" data-idx="${idx}">${row.lineDescription || row.description}</td>
            <td class="amount">
            ...
            <td style="padding: 0%;"><button class="wd-btn reject" onclick="window.deleteModifyServiceBillRow(${idx})" style="width: 100%; height: 100%;">Delete</button></td>
        `;
        tbody.appendChild(tr);
    });
    
    // Minimum rows logic could be added here if desired, but not strictly necessary for modal
    updateModifyServiceBillTotals();
}
...
window.openServiceBillModifyModal = async function(billDetails, refreshCallback) {
    ...
    // Try to find a reference, fallback to transactionType or orderNumber
    const ref = billDetails.transactionType || `Ref-${billDetails.transactionType}`;
    document.getElementById('MSB_BillRef').value = ref;
    
    document.getElementById('MSB_bill_description').value = billDetails.description || '';
    document.getElementById('MSB_inc-ait').checked = billDetails.deductAIT === 1;

    // --- Populate Supplier Info ---
    if (window.SupplierManager) {
        // Ensure we have the latest supplier data to map the ID to a Name
        ...
        if (supplier) {
            ...
            // Fallback if supplier not found in local cache (should be rare)
        ...
        // Fallback if Manager not available (unlikely)
        document.getElementById('MSB_SupplierId').value = billDetails.vendorId;
    }

    // --- Populate Lines ---
    // Map API lines to the format needed by our table: { coaId, accountName, description, amount }
    modifyServiceBillState.data = [];
    if (billDetails.lines && Array.isArray(billDetails.lines)) {
        // We need to resolve account names if they are not provided directly in the line
        if (window.AccountPicker) await window.AccountPicker.ensureData();
 
        modifyServiceBillState.data = billDetails.lines.map(line => {
            // Per user feedback, `lineId` from the API is the Chart of Accounts ID.
            const coaId = line.coaId || 0;
            const description = line.lineDescription || ""; // Line-level description if provided
            let accountName = "Unknown Account";
 
            if (coaId && window.AccountPicker) {
                const account = window.AccountPicker.data.mapById.get(Number(coaId));
                if (account) {
                    accountName = account.name;
                }
            }
            ...
            return {
                ...
                // Description should default to the account name.
...
window.saveModifiedServiceBill = async function() {
    ...
    // 3. Calculate totals (Logic matching create_bills.js)
    ...
    // In create_bills.js, totalVAT stores the AIT amount if deducted
    const aitAmount = deductAIT ? Math.round(totalIOT * 0.055) : 0;
    const totalVAT = aitAmount; 
    
    // Net amount
    const totalIOT = totalBT - totalVAT; 

    // Handle Advance Given (preserve from original bill if exists, else 0)
    //const advanceGiven = Number(originalBill.advanceGiven) || 0;
    //const amountLeft = totalIOT - advanceGiven;

    // 4. Construct payload for update based on provided structure & create_bills.js mapping
    const payload = {
        orderNumber: originalBill.orderNumber,
        ...
        totalIOT: totalIOT,
        totalBT: totalBT,
        totalVAT: totalVAT, // Stores AIT amount
        ...
        lines: modifyServiceBillState.data.map(line => ({
            coaId: line.coaId,
            amount: line.amount,
            lineDescription: line.description || line.lineDescription
        }))
    };
    ...
    try {
        ...
        if (response && response.success) {
            ...
            if (typeof modifyServiceBillState.refreshCallback === 'function') {
                modifyServiceBillState.refreshCallback();
            }
        } else {
        ...
        if (saveButton) { 
            ...
            saveButton.innerHTML = 'Save Changes <i class="fa-solid fa-circle-check"></i>'; 
        }
    }
}
...
window.submitModification = async function() {
    ...
    if (isRejected) {
        if (!confirm("Warning: You are about to resubmit this rejected requisition.\nIt will be moved back to the Pending Approvals list.\nThis action cannot be undone from this screen.\n\nDo you want to proceed?")) {
            return;
        }
    }
...
window.initPendingApproval = function() {
    ...
    const renderRow = (item, index) => {
        ...
        if (item.type === 'payment-order') {
            ...
            btnModify.className = 'wd-btn modify'; // Style from pen_rej_pay.js
            btnModify.textContent = 'Modify';
            btnModify.onclick = () => handleModify(item.rawItem, window.initPendingApproval);
            ...
            btnApprove.onclick = async () => {
                if (!confirm(`Are you sure you want to approve Service Bill #${item.refNo}?`)) return;
                try {
                    ...
                    if (resp && resp.success) {
                        ...
                        window.initPendingApproval(); // Refresh table
                    } else {
            ...
            const btnModify = document.createElement('button');
            btnModify.className = 'wd-btn modify';
            btnModify.textContent = 'Modify';
            btnModify.disabled = false; // Enabling the button
            btnModify.onclick = () => window.handleServiceBillModify(item.rawItem, window.initPendingApproval);
...
window.initRejectedPayments = function() {
    console.log("Initializing Rejected Payments...");
    initializePaymentTable({
        tableId: 'TABLE_RejectedPayments',
        apiEndpoint: '/api/v1/paymentOrders/getRejectedRequisitions',
        refreshCallback: window.initRejectedPayments,
        renderRow: (payment, index) => {
            const tr = document.createElement('tr');
            tr.innerHTML = `
                <td data-tooltip="SN\n${index + 1}">${index + 1}</td>
                ...
                <td data-tooltip="BENEFICIARY\n${formatValue(payment.proposer)}">${formatValue(payment.proposer)}</td>
                <td class="amount" data-tooltip="ACTUAL AMOUNT\n${formatValue(payment.netAmount, 'currency')}">${formatValue(payment.netAmount, 'currency')}</td>
                <td data-tooltip="INITIATED BY\n$System\n${formatValue(payment.initiatedBy)}">${formatValue(payment.initiatedBy)}</td>
                <td data-tooltip="REJECTED BY\n${formatValue(payment.rejectedBy)}">${formatValue(payment.rejectedBy || 'N/A')}</td>
            `;
 
            // Actions Column
            const tdActions = document.createElement('td');
            
            // Only allow delete if not approved/paid (which is true for rejected items usually)
            const btnModify = document.createElement('button');
            btnModify.className = 'wd-btn pay'; // Using 'pay' class for 'Modify' as seen in pen_rej_pay.js
            btnModify.textContent = 'Modify';
            btnModify.onclick = () => handleModify({ ...payment, isRejected: true }, window.initRejectedPayments);

            const btnDelete = document.createElement('button');
            btnDelete.className = 'wd-btn reject';
            btnDelete.textContent = 'Delete';
            btnDelete.onclick = () => handleWorkflowAction('delete', payment.requisitionID, window.initRejectedPayments);
