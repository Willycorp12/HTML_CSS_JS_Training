...
function renderIsTable() {
    ...
    const tfoot = document.querySelector('#IS_Table tfoot');
...
async function openIsChartManager() {
    ...
    document.querySelectorAll('.chart-type-option').forEach(btn => btn.classList.remove('active'));
    document.getElementById(`chart-type-${currentChartType}`).classList.add('active');

    document.querySelectorAll('.legend-controls i').forEach(btn => btn.classList.remove('active'));
    document.getElementById(`BTN_Legend${legendPosition === 'left' ? 'Left' : 'Right'}`).classList.add('active');
    document.getElementById('BTN_ToggleLegend').className = isLegendVisible ? 'fa-solid fa-eye' : 'fa-solid fa-eye-slash';

    // Set initial checkbox states
    document.getElementById('CHART_Smoothing').checked = true; // Default to true
    document.getElementById('CHART_Gradient').checked = true; // Default to true
    document.getElementById('CHART_Animation').checked = true; // Default to true
...
function updateIsChartType(type, element) {
...
function toggleIsChartLegend(element) {
    isLegendVisible = !isLegendVisible;
    if (element) {
        element.className = isLegendVisible ? 'fa-solid fa-eye' : 'fa-solid fa-eye-slash';
    }
    // Control the custom legend div's visibility
    const legendEl = document.getElementById('IS_ChartLegend');
    if (legendEl) {
        legendEl.style.display = isLegendVisible ? 'flex' : 'none';
    }
    renderIsChart();
}

function updateIsChartLegendPos(pos, element) {
    legendPosition = pos;
    document.querySelectorAll('.legend-controls i').forEach(btn => btn.classList.remove('active'));
    element.classList.add('active');
    renderIsChart();
...
function renderIsChart() {
    ...
    const legendEl = document.getElementById('IS_ChartLegend');
    const chartContainer = document.getElementById('IS_ChartContainer');

    if (!canvas || !legendEl || !chartContainer) return;
    ...
    const chartColors = baseColors.map((color, i) => {
        ...
        // Create a radial gradient for each segment
    ...
    let config = {
        ...
        options: {
            ...
            plugins: {
                legend: {
                    display: false // Always hide Chart.js built-in legend in modal
                } 
            }
        }
    };
    ...
    // Apply legend position and visibility to the custom HTML legend
    legendEl.style.display = isLegendVisible ? 'flex' : 'none';
    if (legendPosition === 'left') {
        legendEl.style.order = 1; // Legend first
        chartContainer.style.order = 2; // Chart second
    } else {
        legendEl.style.order = 2; // Legend second
        chartContainer.style.order = 1; // Chart first
    }

    isChartInstance = new Chart(ctx, config);

    // Render Custom Legend HTML
    renderCustomChartLegend(legendEl, labels, values, total, baseColors);
}
...
function renderMainPageFooterChart() {
    ...
    if (!canvas) return;
    canvas.width = 300; canvas.height = 200; // Set explicit size for footer chart
    const ctx = canvas.getContext('2d');
    const revenueItems = activeIsData.filter(d => d.income > 0);
...
function printIsChart() {
    ...
    const legendHtml = document.getElementById('IS_ChartLegend').innerHTML;
    ...
    win.document.write(`
        ...
        <body style="display:flex; flex-direction:column; align-items:center; padding:50px;">
            ...
            <div style="width:400px; margin-top:30px; font-family:sans-serif;">${legendHtml}</div>
